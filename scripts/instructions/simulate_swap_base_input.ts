...
async function main() {
  ...
  try {
    console.log("Building swap transaction (simulation)...");

    // Build the Anchor transaction but don't send it
    ...
    // sign the tx locally with our owner keypair so simulation has signatures
    tx.sign(owner);

    // simulate the signed transaction using VersionedTransaction
    const versioned = VersionedTransaction.deserialize(tx.serialize());
    ...
    if (sim.value?.err) {
      console.error("Simulation reported error:", sim.value.err);
    }
    if (sim.value?.logs) {
