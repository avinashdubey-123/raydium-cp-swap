...
async function main() {
  ...
  // read vault token account to obtain vault mint
  ...
  // recipient accounts (owner's ATAs for vault mint)
  ...
  // ensure recipient ATAs exist (batch)
  const ixs: any[] = [];
  ...
  // fetch on-chain pool state to determine available protocol fees
  const ps: any = await (program.account as any).poolState.fetch(poolState);
  const requested0 = ps.protocolFeesToken0 ?? ps.protocol_fees_token_0 ?? 0;
  const requested1 = ps.protocolFeesToken1 ?? ps.protocol_fees_token_1 ?? 0;

  const sig = await program.methods.collectProtocolFee(new anchor.BN(requested0.toString()), new anchor.BN(requested1.toString()))
    .accounts({
  ...
  console.log("collectProtocolFee tx:", sig);
}
